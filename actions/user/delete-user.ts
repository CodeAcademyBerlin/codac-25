...
type DeleteUserResult = ServerActionResult<{
  id: string;
  email: string | null;
  name: string | null;
}>;
...
export async function deleteUser(
  ...
  try {
    ...
    const hasRelatedData = Object.values(existingUser._count).some(
      count => count > 0
    );
    ...
    if (hasRelatedData) {
      ...
      return {
        success: false,
        error:
          'User has related data and has been deactivated instead of deleted. To permanently delete, remove all related data first.',
      };
    }
