...
const getCohortColor = (cohortName: string) => {
  ...
  if (name.includes('pink') || name.includes('rose') || name.includes('salmon')) return 'rgba(236, 72, 153, 0.8)'
  if (name.includes('yellow')) return 'rgba(234, 179, 8, 0.8)'
  if (name.includes('red') || name.includes('wine')) return 'rgba(239, 68, 68, 0.8)'
  if (name.includes('indigo')) return 'rgba(99, 102, 241, 0.8)'
  if (name.includes('turquoise') || name.includes('cyan')) return 'rgba(6, 182, 212, 0.8)'
  if (name.includes('silver') || name.includes('grey') || name.includes('gray')) return 'rgba(148, 163, 184, 0.8)'
...
function CohortCard({ cohort, index, position, isSelected, onClick, zIndex }: CohortCardProps) {
    ...
    >
      <div
        style={{
          ...
          height: '100%',
          position: 'relative',
          border: isSelected ? '6px solid rgba(255, 255, 255, 0.9)' : '3px solid rgba(255, 255, 255, 0.2)',
          borderRadius: '16px',
      ...
      >
        ...
        >
          ...
          {cohort.start_date && cohort.end_date && (
            ...
            >
              {new Date(cohort.start_date).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })} - {new Date(cohort.end_date).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
            </div>
          )}
        </div>
      </div>
...
export default function CohortPeriodicTableFramer({ cohorts, users }: CohortPeriodicTableFramerProps) {
          ...
          <div className="h-full bg-black/40 backdrop-blur-xl border-l border-white/10 flex flex-col overflow-hidden">
            ...
            {selectedCohortData ? (
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="p-6 border-b border-white/10">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <h2 className="text-2xl font-bold text-white mb-2">{selectedCohortData.name}</h2>
                      {selectedCohortData.start_date && selectedCohortData.end_date && (
                        <p className="text-slate-400 text-sm">
                          {new Date(selectedCohortData.start_date).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })} - {new Date(selectedCohortData.end_date).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
                        </p>
                      )}
                    </div>
                    <button
                ...
                <div className="flex-1 overflow-y-auto p-6">
                  <h3 className="text-lg font-semibold text-white mb-4">
                    Members ({users.filter(u => u.cohort === selectedCohortData.slug).length})
                  </h3>
                  <div className="space-y-3">
                    {users
                      .filter(u => u.cohort === selectedCohortData.slug)
                      .map(user => (
                        <div key={user.username} className="flex items-center gap-3 p-3 bg-white/5 rounded-lg hover:bg-white/10 transition-colors">
                          <img
                            src={`/${user.image}`}
                            alt={user.name}
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                ...
                <h3 className="text-xl font-semibold text-white mb-2">No Cohort Selected</h3>
                <p className="text-slate-400 text-sm max-w-xs">
                  Click on any cohort card in the 3D space to view its details and members.
                </p>
              </div>
            )}
          </div>
