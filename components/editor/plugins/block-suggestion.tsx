        ...
        .flatMap(([node]) => {
          if (TextApi.isText(node)) {
            ...
            const includeUpdate = dataList.some(
              (data: TInlineSuggestionData) => data.type === 'update'
            );

            if (!includeUpdate) return api.suggestion.nodeId(node);

            return dataList
              .filter((data: TInlineSuggestionData) => data.type === 'update')
              .map((d: TInlineSuggestionData) => d.id);
          }
    ...
    suggestionIds.forEach((id) => {
      ...
      entries.forEach(([node]) => {
        if (TextApi.isText(node)) {
          ...
          dataList.forEach((data: TInlineSuggestionData) => {
          ...
          const lineBreakData = api.suggestion.isBlockSuggestion(node)
            ? node.suggestion as TSuggestionData
            : undefined;
